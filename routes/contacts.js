...
router.put('/:id', function(req, res, next){
  EmergencyContacts.findByIdAndUpdate(req.params.id, { $set: { _id:req.params.id, contact:[{name: req.body.name, phone: req.body.phone, email: req.body.email, _id: req.params.id}] }}, function (err, contact) {
    if (err) return next(err);
    contact.save(function(err){
      if (err) res.send('error ' + err);
      else
